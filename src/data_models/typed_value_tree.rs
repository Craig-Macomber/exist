//! TypedTree, but can store value direct in the nodes instead of just children.
//!
//! Associated types were getting too verbose. Shortened to:
//! TN = TypeName
//! CN = ChildName
//!
//! When listing all three, keep them in that order.
//!
//! V = Type parameter that is a Visitor
//! T = Type parameter that is a Tree (might just be a value in a Tree though)

/// Helper super trait for provide the associated types used in this module
pub trait Base {
    type TN;
    type CN;
}

Visitable!(TypeView, TypeVisitor, TN;CN);
Visitable!(MapView, MapVisitor, TN;CN);
Visitable!(ListView, ListVisitor, TN;CN);

pub trait TypeVisitor: Base {
    /// Called if the View was a Struct (map)
    fn visit_map<T: MapView<TN = Self::TN, CN = Self::CN>>(&mut self, type_name: &Self::TN, t: &T);

    /// Called if the View was a Value / Leaf
    // TODO: use ExactSizeIterator?
    fn visit_value(&mut self, type_name: &Self::TN, t: &Vec<u8>);
}

pub trait MapVisitor: Base {
    /// Called for with value in the map
    fn visit<T: ListView<TN = Self::TN, CN = Self::CN>>(&mut self, name: &Self::CN, children: &T);
}

pub trait ListVisitor: Base {
    /// Called for with value in the children list in order
    fn visit<T: TypeView<TN = Self::TN, CN = Self::CN>>(&mut self, child: &T);
}

pub mod concrete {
    use super::*;
    use std::collections::HashMap;
    use std::hash::Hash;

    pub struct Concrete<TN, CN>
    where
        CN: Eq + Hash,
    {
        type_name: TN,
        content: StructOrValue<TN, CN>,
    }

    /// Content helper for Concrete.
    pub enum StructOrValue<TN, CN>
    where
        CN: Eq + Hash,
    {
        Struct(HashMap<CN, Vec<Concrete<TN, CN>>>),
        Value(Vec<u8>),
    }

    impl<TN, CN> TypeView for Concrete<TN, CN>
    where
        CN: Eq + Hash,
    {
        type TN = TN;
        type CN = CN;
        fn visit<V: TypeVisitor<TN = Self::TN, CN = Self::CN>>(&self, v: &mut V) {
            match &self.content {
                StructOrValue::Struct(map) => {
                    v.visit_map(&self.type_name, map);
                }
                StructOrValue::Value(value) => {
                    v.visit_value(&self.type_name, value);
                }
            };
        }
    }

    impl<TN, CN> MapView for HashMap<CN, Vec<Concrete<TN, CN>>>
    where
        CN: Eq + Hash,
    {
        type TN = TN;
        type CN = CN;
        fn visit<V: MapVisitor<TN = Self::TN, CN = Self::CN>>(&self, v: &mut V) {
            for (k, children) in self.iter() {
                v.visit(&k, children);
            }
        }
    }

    impl<TN, CN> ListView for Vec<Concrete<TN, CN>>
    where
        CN: Eq + Hash,
    {
        type TN = TN;
        type CN = CN;
        fn visit<V: ListVisitor<TN = Self::TN, CN = Self::CN>>(&self, v: &mut V) {
            for child in self.iter() {
                v.visit(child);
            }
        }
    }

    /// Copy into the standard Concrete implementation
    pub fn view_to_concrete<T, TN, CN>(t: T) -> Concrete<TN, CN>
    where
        T: TypeView<TN = TN, CN = CN>,

        TN: Clone,
        CN: Clone + Eq + Hash,
    {
        struct Copier<T> {
            t: T,
        }

        fn copier<T>(t: T) -> Copier<T> {
            Copier { t }
        }

        type Co<TN, CN> = Copier<Option<Concrete<TN, CN>>>;
        type Cm<TN, CN> = Copier<HashMap<CN, Vec<Concrete<TN, CN>>>>;
        type Cv<TN, CN> = Copier<Vec<Concrete<TN, CN>>>;

        impl<TN, CN> Base for Co<TN, CN>
        where
            CN: Eq + Hash,
        {
            type TN = TN;
            type CN = CN;
        }

        impl<TN, CN> Base for Cm<TN, CN>
        where
            CN: Eq + Hash,
        {
            type TN = TN;
            type CN = CN;
        }

        impl<TN, CN> Base for Cv<TN, CN>
        where
            CN: Eq + Hash,
        {
            type TN = TN;
            type CN = CN;
        }

        impl<TN, CN> TypeVisitor for Co<TN, CN>
        where
            TN: Clone,
            CN: Clone + Eq + Hash,
        {
            fn visit_map<T: MapView<TN = Self::TN, CN = Self::CN>>(
                &mut self,
                type_name: &Self::TN,
                t: &T,
            ) {
                self.t = Some(Concrete {
                    type_name: type_name.clone(),
                    content: StructOrValue::Struct(t.apply(copier(HashMap::new())).t),
                });
            }
            fn visit_value(&mut self, type_name: &Self::TN, t: &Vec<u8>) {
                self.t = Some(Concrete {
                    type_name: type_name.clone(),
                    content: StructOrValue::Value(t.clone()),
                })
            }
        }

        impl<TN, CN> MapVisitor for Cm<TN, CN>
        where
            TN: Clone,
            CN: Clone + Eq + Hash,
        {
            fn visit<T: ListView<TN = Self::TN, CN = Self::CN>>(
                &mut self,
                name: &Self::CN,
                children: &T,
            ) {
                self.t
                    .insert(name.clone(), children.apply(copier(vec![])).t);
            }
        }

        impl<TN, CN> ListVisitor for Cv<TN, CN>
        where
            TN: Clone,
            CN: Clone + Eq + Hash,
        {
            fn visit<T: TypeView<TN = Self::TN, CN = Self::CN>>(&mut self, child: &T) {
                self.t.push(child.apply(copier(None)).t.unwrap());
            }
        }

        return t.apply(copier(None)).t.unwrap();
    }

    #[cfg(test)]
    mod tests {
        use super::{view_to_concrete, Concrete, StructOrValue};
        use std::collections::HashMap;

        #[test]
        fn test_view_to_concrete() {
            let mut map = HashMap::new();
            map.insert(
                1i128,
                vec![Concrete {
                    type_name: 3i64,
                    content: StructOrValue::Value(vec![1u8, 2u8, 3u8]),
                }],
            );
            let c = Concrete {
                type_name: 2i64,
                content: StructOrValue::Struct(map),
            };

            let c2 = view_to_concrete(c);
            //assert_eq!(c, c2);
        }
    }
}
